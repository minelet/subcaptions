...
module.exports = async (req, res) => {
  ...
  if (action === 'add' && packId) {
    ...
    await sql`
      UPDATE users SET credits = credits + ${pack.credits} WHERE id = ${user.id}
    `;
    const updated = await sql`SELECT credits, credits_infinite FROM users WHERE id = ${user.id}`;
    const row = updated.rows[0];
    return res.status(200).json({ credits: row.credits, creditsInfinite: row.credits_infinite });
  }
  ...
  if (typeof amount === 'number' && amount > 0) {
    ...
    await sql`UPDATE users SET credits = credits - ${amount} WHERE id = ${user.id}`;
