...
// Every branch below only has a user_id (unlike the direct-confirm
// endpoints, which already have the full `user` object from the session)
// — this webhook has no session, it's server-to-server. One small lookup
// beats plumbing email through every query in this file.
  ...
  try {
    if (eventType === 'subscription.charged') {
      ...
      if (already.rows.length === 0) {
        ...
        // This fires for every renewal, not just the first cycle — a
        // monthly "your subscription renewed" receipt, which is correct
        // (unlike the one-time pack backstops below, which only ever fire
        // once per order).
        const toEmail = await getUserEmail(subscriptionRow.user_id);
        const label = SUBSCRIPTIONS[subscriptionRow.plan_key]?.rank || subscriptionRow.plan_key;
        await sendPurchaseReceiptEmail({
          toEmail,
          item: { label, credits: subscriptionRow.credits_per_cycle, priceUsd: subscriptionRow.price_usd, kind: 'subscription' },
          provider: 'razorpay',
        });
      }
    }
  ...
  try {
    ...
    // them to do.
      ...
      if (updated.rows.length > 0) {
        ...
        const toEmail = await getUserEmail(orderRow.user_id);
        await sendPurchaseReceiptEmail({
          toEmail,
          item: { label: pack.rank, credits: pack.credits, priceUsd: pack.priceUsd, kind: 'pack' },
          provider: 'paypal',
