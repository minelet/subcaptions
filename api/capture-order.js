...
// Step 1 of the real PayPal one-time-pack flow: create a PayPal Order
// server-side (fixed price straight from PACKS — never trust a client-sent
// amount) and record a 'pending' row in `orders` right away, while we still
// know which logged-in user this is for. PayPal never hands that back to
// us later — only the order id and whatever custom_id we set below — so
// this pending row is what lets BOTH the capture call (handlePaypalCapture
// below, fired the moment the buyer approves) AND the webhook backstop
// (api/paypal-webhook.js, PAYMENT.CAPTURE.COMPLETED) credit the right
// account without ever asking the buyer to self-report anything.
  ...
  try {
    ...
    const orderRes = await fetch(`${base}/v2/checkout/orders`, {
      ...
      body: JSON.stringify({
        ...
        purchase_units: [{
          custom_id: packId,
          description: `${pack.credits} Litix credits`,
    ...
    if (!order.id) {
      ...
      return res.status(500).json({ error: 'Could not start PayPal checkout' });
    }

    await sql`
      INSERT INTO orders (user_id, paypal_order_id, pack_id, credits, amount_usd, status)
      VALUES (${user.id}, ${order.id}, ${packId}, ${pack.credits}, ${pack.priceUsd}, 'pending')
...
async function handleRazorpayVerify(req, res) {
  ...
  try {
    ...
    // The `inserted.rows.length === 0` check above already confirmed THIS
    // request is the one that actually just granted credits for this
    // order — same guard that prevents double-crediting also prevents
    // double-emailing on a retry/race. Awaited, but never throws (see
    // lib/email.js), so a flaky email provider can't fail a real payment.
...
module.exports = async (req, res) => {
  ...
  if (req.query?.resource === 'paypal-order') return handleCreatePaypalOrder(req, res);
  ...
  // Check if already processed — e.g. the webhook backstop in
  // api/paypal-webhook.js beat this call to it.
  ...
  // If this order was created via handleCreatePaypalOrder above, a pending
  // row already exists and tells us exactly which account it belongs to —
  // refuse to let anyone else capture it.
  const pendingRow = await sql`SELECT user_id FROM orders WHERE paypal_order_id = ${paypalOrderId}`;
  if (pendingRow.rows.length > 0 && String(pendingRow.rows[0].user_id) !== String(user.id)) {
    console.error('PayPal capture: order/user mismatch', { paypalOrderId, ownerId: pendingRow.rows[0].user_id, sessionUserId: user.id });
  ...
  try {
    ...
    // that actually won the write. If two requests for the same order race
    // each other (this call and the webhook backstop both firing), only one
    // gets a row back — the other must not credit the user, or the account
    // gets double-credited.
    //
    // ON CONFLICT DO UPDATE ... WHERE orders.status = 'pending' upgrades the
    // row handleCreatePaypalOrder inserted (the normal path); if no such row
    // exists (e.g. a legacy/direct call) the plain INSERT just creates a
    // completed row outright. Either way, a conflict against an
    // already-'completed' row updates nothing and returns no id.
    const inserted = await sql`
      INSERT INTO orders (user_id, paypal_order_id, pack_id, credits, amount_usd, status)
      VALUES (${user.id}, ${paypalOrderId}, ${customId}, ${pack.credits}, ${pack.priceUsd}, 'completed')
      ON CONFLICT (paypal_order_id) DO UPDATE SET status = 'completed'
    ...
    // Same idempotency guard as the Razorpay branch above (`inserted.rows`
    // just won the write for this order) — safe to email exactly once here.
    await sendPurchaseReceiptEmail({
      toEmail: user.email,
      item: { label: pack.rank, credits: pack.credits, priceUsd: pack.priceUsd, kind: 'pack' },
      provider: 'paypal',
