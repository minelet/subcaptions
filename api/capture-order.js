...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (order.status === 'completed') {
      ...
      return res.json({ ok: true, alreadyProcessed: true, credits: u.rows[0].credits, creditsInfinite: u.rows[0].credits_infinite });
    }
    ...
    res.json({
      ...
      creditsInfinite: updated.rows[0].credits_infinite,
      added: order.credits
    });
  } catch (err) {
    console.error(err);
