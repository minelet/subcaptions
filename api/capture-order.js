...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Add credits, and grant the pack's rank for 1 month from now
    const rankExpiresAt = new Date();
    rankExpiresAt.setMonth(rankExpiresAt.getMonth() + 1);
    await sql`
      UPDATE users
      SET credits = credits + ${pack.credits},
          credits_bought = credits_bought + ${pack.credits},
          rank = ${pack.rank},
          rank_expires_at = ${rankExpiresAt}
      WHERE id = ${user.id}
    `;
