...
async function handleConfirm(req, res) {
  ...
  try {
    ...
    if (already.rows.length === 0) {
      ...
      // Same `already.rows.length === 0` guard that prevents double-crediting
      // (here vs. the webhook backstop) also gates the receipt email.
      await sendPurchaseReceiptEmail({
        toEmail: user.email,
        item: { label: plan.rank, credits: plan.credits, priceUsd: plan.priceUsd, kind: 'subscription' },
        provider: 'paypal',
      });
    }
