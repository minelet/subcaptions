...
const { createSession, setSessionCookie, setCsrfCookie } = require('../../lib/auth');
...
function clearOauthStateCookie(res) {
  appendSetCookie(res, 'oauth_state=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0');
}

// res.setHeader('Set-Cookie', x) called a second time REPLACES the first
// value instead of sending two cookies. This appends onto whatever's already
// queued so both the oauth_state-clear cookie and the session cookie survive.
...
module.exports = async (req, res) => {
  // Same reasoning as google.js: this response sets the session cookie for
  // whichever account was actually chosen in Google's picker. It must never
  // be served from a cache — that's how a switch to a different account can
  // end up looking like it silently logged back into the previous one.
  ...
  try {
    ...
    // SECURITY: log the real error server-side only. This used to append
    // e.message straight onto the redirect URL (?detail=...), which puts
    // exception internals — stack-trace fragments, token/library error
    // strings, sometimes request internals — into the URL bar, browser
    // history, referrer headers, and any analytics/logging that captures
    // full URLs. None of that is meant for whoever's looking over the
    // user's shoulder or has access to those logs. The redirect now carries
    // only a generic, non-identifying error code.
