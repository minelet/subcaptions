...
// (users, orders, clip-finder-stats) were combined into
...
async function handleUsers(req, res) {
  if (req.method === 'GET') {
    ...
    const result = search
      ? await sql`
          SELECT id, email, is_admin, credits, credits_infinite, credits_used, credits_bought, created_at
  ...
  if (req.method === 'PATCH') {
    const { userId, credits, creditsInfinite, infinite } = req.body || {};
