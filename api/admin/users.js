...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    const result = search
      ? await sql`
          SELECT id, email, is_admin, credits, credits_infinite, credits_used, credits_bought, rank, rank_expires_at, created_at
  ...
  if (req.method === 'PATCH') {
    ...
    if (rank !== undefined && credits === undefined && creditsInfinite === undefined && infinite === undefined) {
      const rankExpiresAt = rank ? (() => { const d = new Date(); d.setMonth(d.getMonth() + 1); return d; })() : null;
      await sql`UPDATE users SET rank = ${rank}, rank_expires_at = ${rankExpiresAt} WHERE id = ${userId}`;
