...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { durationSeconds, packId, action } = req.body || {};

    // ADD credits after PayPal payment
    if (action === 'add' && packId) {
      const creditsToAdd = PACKS[packId];
      if (!creditsToAdd) return res.status(400).json({ error: 'Invalid pack' });
      ...
      `;
      return res.json({ ok: true, credits: rows[0].credits, creditsInfinite: rows[0].credits_infinite });
    }
    ...
    if (user.credits < cost) {
      return res.status(402).json({ error: 'Not enough credits', required: cost, available: user.credits });
    }
