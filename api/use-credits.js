...
module.exports = async (req, res) => {
  ...
  // Mid-transcription failover: the client's current Groq key just 401'd
  // or 429'd, so hand back a different one from the pool. `failedKey` gets
  // put on a brief cooldown so we stop handing it out for a bit.
  if (action === 'groq-key') {
    const excludeKeys = Array.isArray(req.body?.excludeKeys) ? req.body.excludeKeys : [];
    if (req.body?.failedKey) markKeyFailed(req.body.failedKey);
    return res.status(200).json({ key: nextAvailableKey(excludeKeys) });
  }
