...
async function runMigrations() {
  ...
  const check = await sql`
    SELECT to_regclass('public.subscription_charges') AS t,
           EXISTS (
             SELECT 1 FROM information_schema.columns
             WHERE table_name = 'subscription_charges' AND column_name = 'razorpay_payment_id'
           ) AS has_razorpay
  `;
  if (check.rows[0]?.t && check.rows[0]?.has_razorpay) return;
  ...
  `;
  // Razorpay one-time-pack orders. Nullable + its own UNIQUE constraint,
  // sitting alongside paypal_order_id rather than replacing it — a given
  // row is either a PayPal order or a Razorpay order, never both.
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS razorpay_order_id TEXT UNIQUE`;
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS provider TEXT NOT NULL DEFAULT 'paypal'`;
  ...
  // Razorpay subscriptions. paypal_subscription_id was NOT NULL because
  // every row used to be PayPal — relaxed so Razorpay-only rows can exist
  // (they'll have razorpay_subscription_id set instead).
  await sql`ALTER TABLE subscriptions ALTER COLUMN paypal_subscription_id DROP NOT NULL`;
  await sql`ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS razorpay_subscription_id TEXT UNIQUE`;
  await sql`ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS provider TEXT NOT NULL DEFAULT 'paypal'`;
  ...
  `;
  // Razorpay charges (first payment at subscribe time + every monthly
  // renewal via webhook). paypal_capture_id relaxed to nullable so a
  // Razorpay-only row doesn't violate it; razorpay_payment_id gets its own
  // partial unique index (only enforced when actually set) so redelivered
  // Razorpay webhooks can't double-credit the same charge, exactly
  // mirroring what the PayPal UNIQUE constraint above already guarantees
  // for PayPal charges.
  await sql`ALTER TABLE subscription_charges ALTER COLUMN paypal_capture_id DROP NOT NULL`;
  await sql`ALTER TABLE subscription_charges ADD COLUMN IF NOT EXISTS razorpay_payment_id TEXT`;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_subscription_charges_razorpay
    ON subscription_charges (subscription_id, razorpay_payment_id)
