...
// Claude to identify which portions are the strongest candidates for
...
async function analyzeTranscriptForClips(segments, totalDurationSec) {
  ...
  for (const chunk of chunks) {
    ...
    let text;
    try {
      text = await callClaude(buildPrompt(chunk));
    } catch (e) {
      // One chunk failing to analyze shouldn't kill the whole video's
      // results — just skip it and keep whatever other chunks produced.
      continue;
    }
