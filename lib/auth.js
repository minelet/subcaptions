...
// actions rely on that alone today. This adds a signed double-submit-cookie
// token as a second layer: a non-HttpOnly cookie holds an HMAC of the
// session id, the admin frontend echoes that value back in an
// X-CSRF-Token header on every mutating request, and the server checks the
// two match. A cross-site page can trigger a cookie-carrying request, but
// it cannot read the cookie's value (browsers enforce same-origin on cookie
// reads) to put into the header, so a forged request fails this check even
// if SameSite were ever misconfigured or bypassed.
//
// The secret only needs to be stable for as long as a warm serverless
// instance lives — a cold start minting a fresh one just means previously
// issued tokens go stale and the client re-fetches a new one via /api/me,
// not a security problem. Set CSRF_SECRET in production if you want tokens
// to survive cold starts.
const CSRF_SECRET = process.env.CSRF_SECRET || crypto.randomBytes(32).toString('hex');

function csrfTokenFor(sessionId) {
  return crypto.createHmac('sha256', CSRF_SECRET).update(String(sessionId)).digest('hex');
}
