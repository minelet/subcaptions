...
async function getSessionUser(sessionId) {
  ...
  const result = await sql`
    SELECT u.id, u.email, u.is_admin, u.credits, u.credits_infinite, u.rank, u.rank_expires_at
    FROM sessions s
  ...
  // Lazily expire a purchased rank once its monthly period is over
  if (row.rank && row.rank_expires_at && new Date(row.rank_expires_at) <= new Date()) {
    await sql`UPDATE users SET rank = NULL, rank_expires_at = NULL WHERE id = ${row.id}`;
    row.rank = null;
    row.rank_expires_at = null;
  }
