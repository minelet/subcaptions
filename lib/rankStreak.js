...
//    decay below). Every purchase increments streak_count by 1 and refreshes
//    the period deadline, no matter how soon it comes after the last one —
//    purchases do not need to be spaced out to stack streak progress.
//  - Missing a period:
...
function applyPurchase(state, packRung, now) {
  ...
  // Step 2: streak math.
  // Every purchase counts toward the streak, no matter how soon it comes
  // after the previous one — there is no "already counted this period"
  // restriction. Each purchase also refreshes the period deadline.
  const newStreakCount = decayed.streakCount + 1;
